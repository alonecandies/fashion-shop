...
export default class MailUtils {
  ...
  public activeAccount(email: string, token: string, user: UserModel = null) {
    try {
      const subject = "HPL Shop Sign up Confirmation";
      const fileTemplate = "activeAccount";
      return new Promise((resolve, reject) => {
        const url = `${
          config.WEBSITE_URL
        }/active-account?token=${encodeURIComponent(token)}`;
        ejs.renderFile(
          `${app_root_path}/src/templates/${fileTemplate}.ejs`,
          { mainUrl: url },
          async (err, data) => {
            if (err) {
              console.log(err);
  ...
  public forgotPassword(email: string, token: string) {
    try {
      const subject = "HPL Shop Password Required";
      let url = `${
        config.WEBSITE_URL
      }/login?action=reset-password&token=${encodeURIComponent(token)}`;
      const response = this._getCommonResponse(email, url);
      return new Promise((resolve, reject) => {
        ejs.renderFile(
          `${app_root_path}/src/templates/forgotPassword.ejs`,
          response,
          async (err, data) => {
            if (err) {
              console.log(err);
