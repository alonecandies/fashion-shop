...
export default function UserStatusEditDialog({ isOpened, idCart, handleClose }) {
  ...
  useEffect(() => {
    if (!!idCart) {
      dispatch(fetchGetCartById({ id: idCart }));
    }
  }, [dispatch, idCart]);

  useEffect(() => {
    cart && setValue(cart.status.toString());
  }, [cart]);

  const handleSubmit = () => {
    dispatch(fetchUpdateCartStatus({ id: cart.id, status: parseInt(value) }));
    handleClose();
  };
          ...
          <FormControl component="fieldset">
            <RadioGroup name="status" value={value} onChange={handleRadioChange}>
              {Object.keys(CART_STATUS).map((item, index) => {
                if (CART_STATUS[item].id !== 0) {
                  return (
                    <FormControlLabel
                      key={index}
                      value={CART_STATUS[item].id.toString()}
                      control={<Radio />}
                      label={CART_STATUS[item].value}
                    />
                  );
                } else return <></>;
              })}
